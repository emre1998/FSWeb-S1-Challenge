...
function indekseGoreFenomen(fenomenler, indeks) {
  const fenomen = fenomenler[indeks];
  return `${indeks}. indekste bulunan fenomen: ${fenomen.profile}`;
}

const sonuc = indekseGoreFenomen(fenomenler, 3);
console.log(sonuc); // 3. indekste bulunan fenomen: Beyonce
...
*/
 ...
 function profilListesi(fenomen) {
  let yeniFenomen = [...fenomen];
  yeniFenomen.forEach((f) => {
    
  });
  return yeniFenomen.map((f) => f.profile);
}
...
ÖRNEK: fenomenSil işlevi fenomenler dizisi ve 0 indeks sayısı ile çağrılırsa, veri kümemizden 'Instagram' kaldırılmış olarak döndürür. */
 function fenomenSil(fenomenler, index) {
  let newFenomenler = [...fenomenler];
  newFenomenler.splice(index, 1);
...
function platformaGoreCokGonderiYapanFenomen(fenomenler, platform) {
  ...
  for (let i = 0; i < fenomenler.length; i++) {
    if (fenomenler[i].platform === platform && fenomenler[i].posts !== 'NA' && fenomenler[i].posts > maxPosts) {
      maxPosts = fenomenler[i].posts;
      maxPostsProfile = fenomenler[i].profile;
    }
  }
 
  return maxPostsProfile;
}
